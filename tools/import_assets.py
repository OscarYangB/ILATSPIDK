...
image_file_text += "#include \"image_structures.h\"\n\n"
...
atlas_enum_text = "enum class Sprite {\n"
atlas_data_text = "constexpr SpriteAtlasTransform sprite_atlas_transform[] {\n"
            ...
            if os.path.isfile(json_path):
                with open(json_path) as data:
                    ...
                    for entry in list:
                        ...
                        atlas_data_text += "\t{%s, %s, %s, %s, %s, %s, %s, %s},\n"%(image_x,image_y,image_w,image_h,aabb[0],aabb[1],aabb[2],aabb[3])
                        atlas_to_image_text += "\tImageFile::%s,\n"%(asset_name)
            else:
                atlas_enum_text += "\t" + name.upper() + ",\n"
                image = Image.open(full_path)
                aabb = image.getbbox()
                assert aabb is not None
                atlas_data_text += "\t{%s, %s, %s, %s, %s, %s, %s, %s},\n"%(0, 0, image.size[0], image.size[1],aabb[0],aabb[1],aabb[2],aabb[3])
                atlas_to_image_text += "\tImageFile::%s,\n"%(asset_name)
